...
class FSSM(nn.Module):
    def __init__(
        ...
        self.dz = stoch_trans(dim_z, dim_h1+dim_h2, dim_dz)
        self.dx = stoch_trans(dim_x, dim_h1, dim_dx)
    ...
    def model(self, mini_batch_x, annealing_factor=1.0):
        ...
        with pyro.plate("z_minibatch", batch_size):
            for t in pyro.markov(range(1, T_max + 1)):
                # p(z_t|h_{t-1}^1, h_{t-1}^2)
                if t == 1:
                    dz_input = torch.cat((h1_prev[0,:,:], h2_0_contig[0,:,:]), dim=-1)
                else:
                    dz_input = torch.cat((h1_prev[0,:,:], h2_output[:,t-2,:]), dim=-1)
                ...
                # p(x_t|h_t^2)
                x_loc, x_scale = self.dx(h1_prev[0,:,:])
...
def reconstruct_fssm(model_trained, x_data, guide_1=True, num_MC=1000):
